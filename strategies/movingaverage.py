"""
Moving Average strategy and simple backtester.

Rules (daily timeframe):
- Symbol: default 'SPY' (S&P 500 proxy)
- Go long when Close > N-day simple moving average (SMA)
- Exit to cash when Close < SMA
...
def _compute_ma_signals(df: pd.DataFrame, ma_period: int = 200) -> pd.DataFrame:
    df = df.copy()
...
def backtest_ma(
    ...
    """Run a Moving Average backtest for the given symbol.
    ...
    with_signals = _compute_ma_signals(raw, ma_period)
...
def run_cli(
    ...
    res = backtest_ma(symbol=symbol, years=years, initial_cash=initial_cash, ma_period=ma_period)
    s = res.summary
    print(f"MA{ma_period} Backtest Summary")
    print(f"Symbol: {s['symbol']}  MA Period: {s['ma_period']}")
...
def plot_backtest(result: BacktestResult, save: Optional[str] = None, show: bool = True) -> None:
    ...
    ax1.set_title(f"MA{ma_period} Backtest {result.symbol}: {result.start.date()} → {result.end.date()}")
    ax1.legend(loc='best')
